...
build-docker:
	docker build --build-arg="VERSION=testing" --build-arg="REVISION=n/a" -t bambuvn/filebrowser -f _docker/Dockerfile .

build-docker-slim:
	docker build --build-arg="VERSION=testing" --build-arg="REVISION=n/a" -t bambuvn/filebrowser:slim -f _docker/Dockerfile.slim .
...
build-docker-slim: setup-buildx
	...
	docker buildx build \
		...
		--push \
		.
	@echo "✓ Slim multi-platform image pushed: bambuvn/filebrowser:latest"(Fix issue cannot keep api token alive)
